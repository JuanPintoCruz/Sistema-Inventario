...
if($_POST){
    ...
    $foto_portada1 = isset($_FILES["foto_portada1"]["name"]) ? $_FILES["foto_portada1"]["name"] : "";

    // Prepara la consulta SQL para insertar los datos del empleado
    $sentencia = $conexion->prepare("INSERT INTO `tbl_banners` 
    (`titulo1`, `descripcion1`, `foto_portada1`) 
    VALUES (:titulo1, :descripcion1, :foto_portada1)");
    ...
    $nombreArchivo_foto = ($foto_portada1 != '') ? $fecha_actual->getTimestamp() . "_" . $foto_portada1 : "";
    // Obtener la ubicación temporal del archivo cargado
    $tmp_foto = isset($_FILES["foto_portada1"]["tmp_name"]) ? $_FILES["foto_portada1"]["tmp_name"] : "";
    ...
    $sentencia->bindParam(":foto_portada1", $nombreArchivo_foto);
    $sentencia->execute();
...
$sentencia_banners = $conexion->prepare("SELECT * FROM `tbl_banners`");
$sentencia_banners->execute();
$lista_tbl_banners = $sentencia_banners->fetchAll(PDO::FETCH_ASSOC);
?>
        ...
        <div class="row">
          ...
          <div class="col-md-4">
            <div class="form-group">
              ...
              <input type="text" class="form-control" id="titulo1" name="titulo1" placeholder="Ingrese el primer titulo">
            </div>
            <div class="form-group">
              <label for="descripcion1">Descripcion:</label>
              <input type="text" class="form-control" id="descripcion1" name="descripcion1" placeholder="Ingrese la primera descripcion1">
            </div>
            <div class="form-group">
              <label for="foto_portada1">Foto  de Portada 1:</label>
              <input type="file" id="foto_portada1" name="foto_portada1" class="form-control-file">
            </div>
          </div>
          <div class="col-md-4"></div>
        </div>
